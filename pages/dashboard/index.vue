...
const activeProject = computed(() =>
  workspaceStore.getActiveWorkspace?.projects?.find(
    (project: Project) =>
      project.id === userStore.user?.activeProjectId && !project.archivedAt,
  ),
);
...
watch(requestedProjectId, async (projectId) => {
  if (!projectId || projectId !== activeProject.value?.id) return;
  consumeRename(projectId);
  await startEditing();
});
...
watch(
  ...
  async (projectId, previousId) => {
    if (projectId && requestedProjectId.value === projectId) {
      consumeRename(projectId);
      await startEditing();
      return;
    }
