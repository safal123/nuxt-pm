...
const view = ref<"sent" | "templates">("sent");
...
watch([projectId, templateId, () => workspaceStore.activeWorkspaceId], () => {
  page.value = 1;
  fetchEmails();
});
...
const onView = (value: unknown) => {
  if (value === "sent" || value === "templates") view.value = value;
};
...
<template>
  <div class="h-full min-w-0">
    ...
    <div v-if="view === 'sent'" class="space-y-4">
        ...
        <Table>
          <TableHeader>
            <TableRow class="hover:bg-transparent border-border">
              <TableHead class="h-10">To</TableHead>
