---
name: resend
description: Send email with the official Resend Node.js SDK. Use when sending email, workspace invites, notifications, or installing/configuring Resend (RESEND_API_KEY, resend.emails.send).
---
...
7. **Use camelCase** for SDK parameters (`replyTo`, `scheduledAt`), not snake_case.
...
3. **Do not** use snake_case parameter names (`reply_to`, `scheduled_at`) — the Node.js SDK uses camelCase (`replyTo`, `scheduledAt`).
...
- Read `RESEND_API_KEY` and `RESEND_FROM` from the environment. `RESEND_FROM` must be a verified domain address such as `Northstar <invites@yourdomain.com>`.
- Send from server utilities only (`server/utils/email.ts`). This app is Nuxt/Vue — use `html` and `text`, not `react`.
- Workspace invite emails use idempotency key `workspace-invite/<inviteId>`.
